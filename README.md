...
`config.json` の `notifications.enabled` を `false` にすると、アラートと定時レポートの
**LINE送信だけを停止**できる(測定・記録・閾値判定・ダッシュボード表示は今まで通り継続)。
停止中は通知記録を残さないため、`true` に戻せば発火中のアラートはそのまま再通知される。
...
| `notifications` | `{ enabled }` LINE通知全体のキルスイッチ。`false` でアラート・定時レポートの送信を停止(測定・記録・ダッシュボード表示は継続)。死活監視は `watchdog.enabled` で別管理 |
