...
- 同じリポジトリ・同じ期間で並走している外部スケジューラ（cron-job.org → `workflow_dispatch`）は、15分間隔の **96.7% が ±1分以内**、欠測 3.0%
...
温湿度の収集（15分間隔）は、**GitHub cron ではなく外部スケジューラ（cron-job.org）から `workflow_dispatch` API を叩いて**起動しています。組み始めた当初は `schedule` を使っていたのですが、間隔が全く安定せず、早々に乗り換えました。
    ...
    steps:
      ...
      - if: always()
        run: |
          ...
          git add data/
          git diff --cached --quiet || git commit -m "data: record $(date -u +'%Y-%m-%dT%H:%MZ')"
          git push
```
...
`git diff --cached --quiet ||` を挟んでいるのは、**差分が無いときに commit が失敗してジョブが赤くなるのを防ぐため**です。これが無いと失敗通知が飛び続けます。

コミットステップに `if: always()` を付けているのは、LINE 通知に失敗しても測定データだけは残したいからです。
...
`:00` を避けて `:23` にしているのは、キリのいい時刻ほど混むだろうという素朴な予想からです。結果的にこれは**ほとんど意味がありませんでした**（後述）。
...
40〜45分のバケットが最頻値（74件）で、15〜20分（63件）がそれに次ぎます。二峰性に見えますが、後述の時間帯別の偏りを均した結果です。
...
**UTC 03-09時に集中。** 散発的ではなく、時間帯に紐づいた偏りです。欧州の業務時間帯と米国東海岸の朝が重なるあたりで、CI が最も混む時間帯という説明は納得できます。

曜日別も見ましたが、平日 30-32分 / 土日 24-25分 で、**時間帯ほど大きな差ではありませんでした**（サンプルが半月ぶんしかないので、これは断定しません）。

**「15分間隔の設定にする意味はあったか」という問いについて。** これは測るまでもなく答えが出ていて、遅延の中央値が30分ある以上、**cron に15分間隔を指定しても15分間隔にはなりません**。実行間隔の中央値は 59.5分、最大 131.5分でした。毎時指定でこれなので、`*/15` を指定しても間引かれて似たような結果になるはずです。

## 比較：外部スケジューラ + workflow_dispatch

同じリポジトリの収集側（cron-job.org → `workflow_dispatch`）を、同じ期間で測ったものです。こちらは記録データのタイムスタンプそのものから間隔を出しています。

| 指標 | GitHub cron（毎時指定） | 外部スケジューラ（15分指定） |
|---|---|---|
| 件数 | 358 | 1,738 |
| 期間 | 16.55日 | 18.65日 |
| 間隔の中央値 | 59.5分 | **15.07分** |
...
**指定間隔の ±1分に 96.7% が収まります。** GitHub cron が中央値で30分ずれるのに対して、桁が2つ違う精度です。

欠測3.0%のほうは、外部スケジューラの失敗というより `workflow_dispatch` API 呼び出しやランナー起動の失敗を含んだ数字です。それでも GitHub cron の 10.1% より大きく低い。

要するに、**「起動をGitHubに任せるか外に出すか」だけで精度が2桁変わります。** 無料の外部 cron サービス1つで済むので、精度が要るなら迷う必要は無さそうです。
...
- **`schedule` はデフォルトブランチでしか動かない。** ブランチで cron を書いてもテストできません（これは実際にハマりました。`workflow_dispatch` を併記して手動で確認するのが確実です）
...
- 精度が必要なら、**起動だけ外部スケジューラに出して `workflow_dispatch` を叩く**。同じ条件で ±1分に 96.7% が収まりました
